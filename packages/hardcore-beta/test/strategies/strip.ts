...
describe("HegicPoolStrip", async () => {
  ...
  beforeEach(async () => {
    ...
    const {alice, hegicStripETH, hegicStripBTC, pricerBTC, pricerETH} =
      contracts
  ...
  describe("ETH", async () => {
    ...
    it("less then min period", async () => {
      const {alice, WETH, ethPriceFeed, hegicStripETH, pricerETH, deployer} =
        contracts
      await hegicStripETH.setLimit(
        ethers.utils.parseUnits("1000000", await contracts.USDC.decimals()),
      )
    ...
    it("locked amount", async () => {
      ...
      ).to.be.eq(BN.from(2002.238041e6))
    })
      ...
      await expect(
        hegicStripETH
          ...
          .buy(
            await alice.getAddress(),
            86400 * 30,
            BN.from(ethers.utils.parseUnits("1724", await WETH.decimals())),
            0,
          ),
  ...
  describe("BTC", async () => {
    ...
    it("less then min period", async () => {
      const {alice, WBTC, btcPriceFeed, hegicStripBTC, pricerBTC, deployer} =
        contracts
      await hegicStripBTC.setLimit(
        ethers.utils.parseUnits("1000000", await contracts.USDC.decimals()),
      )
    ...
    it("locked amount", async () => {
      ...
      ).to.be.eq(BN.from(17341.458447e6))
    })
      ...
      await expect(
        hegicStripBTC
          ...
          .buy(
            await alice.getAddress(),
            86400 * 30,
            BN.from(ethers.utils.parseUnits("121", await WBTC.decimals())),
            0,
          ),
      ).to.be.revertedWith("HegicStrategy: The limit is exceeded")
    })
