...
describe("HegicPoolStaddle", async () => {
  ...
  describe("ETH", async () => {
    ...
    it("less then min period", async () => {
      const {alice, deployer, WETH, ethPriceFeed, hegicStraddleETH, pricerETH} =
        contracts
      await hegicStraddleETH.setLimit(
        ethers.utils.parseUnits("1000000", await contracts.USDC.decimals()),
      )
    ...
    it("locked amount", async () => {
      ...
      ).to.be.eq(BN.from(1320.166511e6))
    })
      ...
      await expect(
        hegicStraddleETH
          ...
          .buy(
            await alice.getAddress(),
            86400 * 30,
            BN.from(ethers.utils.parseUnits("1580.45", await WETH.decimals())),
            0,
          ),
  ...
  describe("BTC", async () => {
    ...
    it("less then min period", async () => {
      const {alice, deployer, WBTC, btcPriceFeed, hegicStraddleBTC, pricerBTC} =
        contracts
      await hegicStraddleBTC.setLimit(
        ethers.utils.parseUnits("1000000", await contracts.USDC.decimals()),
      )
    ...
    it("locked amount", async () => {
      ...
      ).to.be.eq(BN.from(11406.177507e6))
    })
      ...
      await expect(
        hegicStraddleBTC
          ...
          .buy(
            await alice.getAddress(),
            86400 * 30,
            BN.from(ethers.utils.parseUnits("185", await WBTC.decimals())),
            0,
          ),
      ).to.be.revertedWith("HegicStrategy: The limit is exceeded")
    })
