...
async function deployment(hre: HardhatRuntimeEnvironment): Promise<void> {
  ...
  async function deploySTRIP(params: typeof paramsETH) {
    const pricerName = `PriceCalculatorSTRIP_${params.currency}`
    const strategyName = `HegicStrategySTRIP_${params.currency}`
...
deployment.tags = ["test-single", "single-strip"]
